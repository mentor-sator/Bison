...
The files already in the working directory:

The WORKSPACE FILES block in your context lists what is on disk inside the
working directory right now: one line per file, with its size in bytes and,
for a Python file, the names it defines at the top level. A file listed there
exists. A file not listed there does not, whatever the task description
implies.

Code you write must use the names those files actually define. When a step
imports from a file in the block, it imports only names that file is listed as
defining. When the name you want is not there, write it in this plan rather
than assume it, and put that write before anything that uses it. A step that
imports a name no file defines is a step that fails the moment it runs.

When the working directory is empty the block says so, and every file the task
needs is one you plan. When there are more files than the block can carry, it
says how many of how many it lists.
...
Ports 8000 to 9000 belong to BISON's own services on this machine. Never bind
one, in a file you write or in an argument you pass. Bind a port above 9000,
such as 9101. A plan that takes one of BISON's ports is sent back to you with
the step named.

A plan exists to make its task's criteria true. Writing a program does not
make true anything the program would do when it runs. A criterion that checks
a row in a database, a file the program produces, or a port it opens needs a
run_python_script or run_python_module step that runs the program, placed after
the step that writes it, with that criterion's id in the run step's
criterion_refs. A plan that writes a program and never runs it leaves those
criteria failed.
